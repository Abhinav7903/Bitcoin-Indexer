...
func NewWriter(pool *pgxpool.Pool, historicalSync bool) *Writer {
	return &Writer{pool: pool, historicalSync: historicalSync}
}
...
func (w *Writer) SaveBlockBatch(
	...
	// synchronous_commit=off: WAL is written to OS buffer but not
	// fsynced before commit returns. Safe because every batch is
	// idempotent (ON CONFLICT DO NOTHING / DO UPDATE). Worst case
	// on crash: re-index the last batch.
	if _, err := tx.Exec(ctx, "SET LOCAL synchronous_commit = OFF"); err != nil {
		return fmt.Errorf("set synchronous_commit: %w", err)
	}

	// ----------------------------------------------------------
	// 1. Immutable chain data
	// ----------------------------------------------------------
	...
	// ----------------------------------------------------------
	// 2. Spend state — mark outputs as spent, delete from utxo_set
	// ----------------------------------------------------------
	...
	// ----------------------------------------------------------
	// 3. Address data — skipped during historical sync.
	//    The backfill tool (cmd/backfill/main.go) reconstructs
	//    address_transactions and address_balances after the
	//    indexer reaches chain tip.
	// ----------------------------------------------------------

	if !w.historicalSync {

		// Receiver rows (outputs with addresses)
		if err := copyReceiverAddressTransactions(ctx, tx, addrTxs); err != nil {
			return err
		}

		// Sender rows (spent outputs → spending tx)
		if spentInputsReady {
...
func copyBlocks(ctx context.Context, tx pgx.Tx, blocks []models.Block) error {
	_, err := tx.CopyFrom(
		...
		[]string{
			...
			"block_time", "bits", "nonce", "version",
			"tx_count", "size_bytes", "weight", "total_fees_sats",
		},
		pgx.CopyFromSlice(len(blocks), func(i int) ([]interface{}, error) {
			b := blocks[i]
			return []interface{}{
				b.Height, b.Hash, b.PreviousHash, b.MerkleRoot,
				b.Time, b.Bits, b.Nonce, b.Version,
				b.TxCount, b.SizeBytes, b.Weight, b.TotalFeesSats,
			}, nil
		}),
	)
	if err != nil {
...
func copyTransactions(ctx context.Context, tx pgx.Tx, txs []models.Transaction) error {
	...
	_, err := tx.CopyFrom(
		...
		[]string{
			"txid", "block_height", "block_hash", "tx_index",
			"version", "locktime", "is_coinbase",
			"input_count", "output_count", "fee_sats",
			"size_bytes", "vsize", "weight", "has_segwit",
		},
		pgx.CopyFromSlice(len(txs), func(i int) ([]interface{}, error) {
			t := txs[i]
			return []interface{}{
				t.Txid, t.BlockHeight, t.BlockHash, t.TxIndex,
				t.Version, t.Locktime, t.IsCoinbase,
				t.InputCount, t.OutputCount, t.FeeSats,
				nullableInt32(t.SizeBytes), nullableInt32(t.VSize),
...
// ============================================================
// copyOutputs — writes tx_outputs + utxo_set
// ============================================================
...
func copyOutputs(ctx context.Context, tx pgx.Tx, outputs []models.Output) error {
	...
	_, err := tx.CopyFrom(
		...
		[]string{
			"txid", "vout_idx", "block_height",
			"value_sats", "script_pubkey", "script_type", "address",
		},
		pgx.CopyFromSlice(len(outputs), func(i int) ([]interface{}, error) {
			o := outputs[i]
			return []interface{}{
				o.Txid, o.VoutIdx, o.BlockHeight,
				o.ValueSats, nullableBytes(o.ScriptPubKey),
				o.ScriptType, nullableString(o.Address),
			}, nil
		}),
	)
	if err != nil {
	...
	_, err = tx.CopyFrom(
		...
		pgx.CopyFromSlice(len(utxos), func(i int) ([]interface{}, error) {
			o := utxos[i]
			return []interface{}{
				o.Txid, o.VoutIdx, o.Address,
				o.ValueSats, o.BlockHeight, o.ScriptType,
			}, nil
		}),
	)
	if err != nil {
...
// ============================================================
// copyInputs — writes tx_inputs + builds temp_spent_inputs
// ============================================================
...
func copyInputs(ctx context.Context, tx pgx.Tx, inputs []models.Input) (bool, error) {
	...
	_, err := tx.CopyFrom(
		...
		[]string{
			"txid", "vin_idx", "block_height",
			"prev_txid", "prev_vout",
			"script_sig", "witness_data", "sequence_no",
		},
	...
	// ----------------------------------------------------------
	// Temp table for spend state.
	// NO prev_block_height column — we removed the per-row
	// UPDATE that resolved it (caused 12-partition fan-out).
	// applySpendState uses a forced hash join instead, which
	// scans each partition once in bulk.
	// ----------------------------------------------------------
...
`); err != nil {
	...
	// Index for hash join performance on the temp side.
	if _, err = tx.Exec(ctx, `
...
// applySpendState
//
// KEY INSIGHT: We removed the expensive per-row UPDATE that
// resolved prev_block_height (caused 17,000 × 12 partition
// lookups per batch). Instead we force a HASH JOIN which
// builds a hash table of temp_spent_inputs once, then does a
// single sequential pass over each tx_outputs partition.
// This turns O(inputs × partitions) into O(partitions).
// ============================================================

func applySpendState(ctx context.Context, tx pgx.Tx) error {

	// Force hash join — Postgres must NOT use nested loops here
	// because that causes one index lookup per input row × 12
	// partitions = hundreds of thousands of random IOs per batch.
	// Hash join: build hash table from temp_spent_inputs once,
	// scan each partition once, emit matches. Much faster on HDD/SSD.
	if _, err := tx.Exec(ctx, `
...
`); err != nil {
	...
	// Restore planner defaults for subsequent queries in this tx.
	if _, err := tx.Exec(ctx, `
SET LOCAL enable_nestloop  = ON;
SET LOCAL enable_mergejoin = ON;
`); err != nil {
		return fmt.Errorf("restore planner hints: %w", err)
	}

	// Delete spent UTXOs from the hot unpartitioned table.
	if _, err := tx.Exec(ctx, `
...
// Derives sender rows from temp_spent_inputs + tx_outputs.
// Uses hash join (nestloop disabled above still in effect).
// ============================================================
...
GROUP BY
	o.address, s.spent_height, t.tx_index, s.spending_txid, b.block_time
...
func updateAddressBalances(
	...
	// Temp delta table — one row per (address, block_height, direction)
	if _, err := tx.Exec(ctx, `
...
`); err != nil {
	...
	// Sender deltas from temp_spent_inputs
	if includeSenders {
		if _, err := tx.Exec(ctx, `
...
`); err != nil {
	...
	// Index for GROUP BY performance
	if _, err := tx.Exec(ctx, `
...
`); err != nil {
	...
	// Pre-aggregate into a second temp table so the upsert
	// touches one row per address, not one per (address, block).
	if _, err := tx.Exec(ctx, `
...
`); err != nil {
	...
	// Upsert into address_balances
	if _, err := tx.Exec(ctx, `
INSERT INTO address_balances (
	address,
	balance_sats, total_received_sats, total_sent_sats,
	utxo_count, tx_count,
	first_seen_height, last_seen_height, updated_at_height
)
SELECT
	address,
	balance_delta, received_delta, sent_delta,
	utxo_count, tx_delta,
...
// ============================================================
// ensurePartitions — auto-creates partition tables ahead of time
// ============================================================
...
func (w *Writer) ensurePartitions(ctx context.Context, blocks []models.Block) error {
	...
	// Pre-create next partition when within 1000 blocks of boundary
